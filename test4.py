...
Validation of FuzzMDA on real-world data sets.
"""
import sys
...
for i in range(N):
    ...
    nlist = []
    for j in range(m):
        result[0, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
        else:
            nlist.append(j)
    ...
    dlist = []
    for j in range(m):
        result[1, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
    for j in dlist:
        jj = (j + 1) % m
    ...
    nlist = []
    for j in range(m):
        result[2, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
        else:
            nlist.append(j)
    ...
    dlist = []
    for j in range(m):
        result[3, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
    for j in dlist:
        jj = (j + 1) % m
    ...
    nlist = []
    for j in range(m):
        result[4, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
        else:
            nlist.append(j)
    ...
    dlist = []
    for j in range(m):
        result[5, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
    for j in dlist:
        jj = (j + 1) % m
    ...
    nlist = []
    for j in range(m):
        result[6, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
        else:
            nlist.append(j)
    ...
    dlist = []
    for j in range(m):
        result[7, j, i], drift = hdlr.score(x[:, j, :], y[:, j])
        if drift:
            dlist.append(j)
    for j in dlist:
        jj = (j + 1) % m
...
with open('result3.weather.{:3d}.npy'.format(batch_size), 'wb') as f:
    np.save(f, result)
