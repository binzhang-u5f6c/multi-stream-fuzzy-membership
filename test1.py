...
if __name__ == "__main__":
    ...
    for i in range(12):
        plt.subplot(3, 4, i+1)
        plt.title('Sythetic{:02d}'.format(i+1))
        plt.xlabel('Size of training set')
        plt.ylabel('mean squared error')
        plt.xticks(list(range(6)), [10, 20, 50, 100, 200, 500])
        plt.plot(scores[i, :, 0, :].mean(axis=1), label='with_n')
        plt.plot(scores[i, :, 1, :].mean(axis=1), label='with_1')
        plt.plot(scores[i, :, 2, :].mean(axis=1), label='with_2')
        plt.plot(scores[i, :, 3, :].mean(axis=1), label='with_3')
        plt.plot(scores[i, :, 4, :].mean(axis=1), label='with_a')
        plt.legend()
    plt.subplots_adjust(top=3, bottom=0, left=0, right=4)
    plt.savefig('exp1synthetic.png')
